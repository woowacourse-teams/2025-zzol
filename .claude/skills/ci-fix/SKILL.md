---
name: ci-fix
description: PR의 backend CI 실패 테스트를 감지하고 수정 사이클을 진행한다. 루트·backend 어느 폴더에서든 실행된다.
...
루트 스킬이라 어느 폴더에서 세션을 열어도 뜬다. 아래 경로와 명령은 모두 저장소 루트 기준이라 `backend/`에서 열어도 그대로 동작한다.
...
```bash
bash "$(git rev-parse --show-toplevel)/.claude/skills/ci-fix/fetch-failures.sh" <PR번호>
```
...
4. `backend/gradlew -p backend :해당모듈:test --tests "coffeeshout.해당패키지.해당테스트클래스" --build-cache` 로 단일 테스트 통과 확인
   - 실패 시 `backend/**/build/test-results/**/*.xml` 에서 `<failure>` 또는 `<error>` 파일을 찾아 재분석
   - 2회 이상 실패 시 사용자에게 보고하고 처리 방법(계속 수정 / 건너뜀 / 중단)을 선택하게 한다

## Step 5: 전체 회귀 확인

모든 수정 완료 후 영향 받은 모듈만 `backend/gradlew -p backend :해당모듈:test --build-cache`로 확인한다. 전체 `test`는 직접 돌리지 않는다(근거는 `backend/CLAUDE.md` 테스트 실행 규칙). 실패 시 Step 4와 동일하게 XML을 분석한다.

## Step 6: 커밋

`backend/`에서 연 세션이면 `/commit`을 호출한다. 루트에서 연 세션에는 그 스킬이 없으므로 `[fix] …` 컨벤션으로 직접 커밋한다.
