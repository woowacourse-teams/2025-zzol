...
제목은 간략한 설명 한 줄이다(SKILL.md "PR 제목"). 줄표 뒤에 세부를 이어 붙이면 목록 화면에서 잘리고 본문과 두 벌이 된다.

| ❌ | ✅ |
|----|----|
| `[fix] 레이싱 게임 서버 정확성 — Runner 동기화, 시작 순서, 속도 상한 검증` | `[fix] 레이싱 게임 서버의 Runner 동기화 오류 수정` |
| `[chore] 백엔드 규칙 기계 검증 도입 — 스케줄러·YAML 미러 테스트, ArchUnit 네이밍` | `[chore] 백엔드 규칙을 테스트로 검증하게 만들기` |
| `[feat] 레이싱 게임 프론트엔드 1단계 개선 — 서버 변경 없이 가능한 28건` | `[feat] 레이싱 게임 프론트엔드 1단계 개선` |

세 ❌ 모두 줄표 뒤가 본문 `🚀 작업 내용`의 변경 목록과 겹친다. 제목에서 지워도 잃는 정보가 없다.
