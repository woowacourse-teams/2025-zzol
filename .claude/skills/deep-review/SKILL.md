...
대상 파일을 `src/main/java`·`frontend/src`로 **하드코딩하지 않는다.** 그러면 `build.gradle`·설정·리소스 변경이 누락된다. 스택 경로(`-- backend/`·`-- frontend/`)로 스코프한 실제 diff 목록을 넘긴다.
...
CodeRabbit이 실제로 해주던 일은 노이즈 필터다. 병합 결과를 채점 에이전트 **1개**에 넘겨 항목별 0-100점을 받고 **80점 미만은 버린다**. 발견사항별 팬아웃은 하지 않는다. 에이전트 수만 늘고 얻는 게 없다.
