...
- **새 게임이 전용 스케줄러(`@Profile("!test")` 빈)·전용 Redis Stream 키를 쓰면** 테스트 미러를 같은 커밋에 추가: `IntegrationSchedulerTestConfig`에 동일 이름 `ShutDownTestScheduler` 빈, `application-test-base.yml`의 `redis.stream.keys`에 동일 키. 빠뜨리면 `:app`의 `SchedulerMirrorTest`·`ConfigMirrorTest`가 잡는다(Docker 불필요). 어디를 고칠지는 `game-test-mirror.md`
