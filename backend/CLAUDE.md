...
- PR 타깃은 `be/dev`이다
- `.github/` 워크플로우(`backend-ci.yml`·`docs-ci.yml` 등)·CodeRabbit 설정 등 GitHub 관련 파일도 `be/dev` PR에서 트리거·검증된다. 따라서 이 파일 변경도 다른 작업과 동일하게 **`be/dev`를 타깃으로** 작업한다(`main` 직접 push·커밋은 git-push-safety 규칙대로 금지)
