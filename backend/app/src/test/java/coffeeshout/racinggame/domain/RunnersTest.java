...
class RunnersTest {
    ...
    final Gamer 한스 = GamerFixture.게스트한스();
    final Gamer 꾹이 = GamerFixture.게스트꾹이();
    final List<Gamer> gamers = List.of(한스, 꾹이);
    final Runners runners = new Runners(gamers);

    @Test
    void 플레이어의_속도를_업데이트할_수_있다() {
        // when
        runners.updateSpeed(gamers.getFirst(), 5, speedCalculator, Instant.now());
    ...
    void 우승자를_찾을_수_있다() {
        ...
        runners.updateSpeed(gamers.getFirst(), 10, speedCalculator, now);
        for (int i = 0; i < 100; i++) {
            runners.moveAll(now);
        }
        ...
        // then
        assertThat(winner.getGamer()).isEqualTo(gamers.getFirst());
    }
    ...
    void 모든_러너의_속도를_조회할_수_있다() {
        // given
        runners.updateSpeed(gamers.getFirst(), 8, speedCalculator, Instant.now());
        runners.updateSpeed(gamers.get(1), 8, speedCalculator, Instant.now());
    ...
    void 모든_러너가_완주했는지_확인할_수_있다() {
        ...
        final Instant now = Instant.now();
        runners.updateSpeed(gamers.getFirst(), 10, speedCalculator, now);
        runners.updateSpeed(gamers.get(1), 10, speedCalculator, now);
        for (int i = 0; i < 100; i++) {
            runners.moveAll(now);
        }
