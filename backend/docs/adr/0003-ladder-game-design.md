...
### 1. 선 배치 알고리즘 — 전역 삽입 순서 기반 row 할당

서버가 선을 수신한 전역 순서가 수직 위치(row)를 결정한다.

```text
add(playerName, segmentIndex S):
  row = 현재까지 추가된 선의 수 + 1  (전역 삽입 순서)
  insert LadderLine(playerName, S, row)
```

플레이어 1인당 1선 제약 하에서 전역 삽입 순서는 row를 항상 유일하게 만든다. 따라서 "같은
높이에 인접 두 선"이 수학적으로 불가능하며, 인접 충돌 감지·해소 로직이 불필요하다.

누른 순서가 사다리의 위→아래 위치에 그대로 반영된다.
...
경로 계산은 서버가 담당하여 최종 순위를 확정한다. `LadderFlowOrchestrator`는 먼저
`changeToResult()`로 상태를 전환하여 이후 선 추가 이벤트를 차단한 뒤, `tracePaths()`를
호출하여 순위를 확정한다.
...
결정 1과 결정 3을 바꾼다. 플레이어가 선을 **3개까지** 긋고, 높이(row)를 **직접 고른다.**

한 사람이 선 하나를 받은 순서대로 긋는 방식은 선이 적고 어디에 긋는지 고를 여지가 없었다. 규칙안 8개를 비교한 끝에 여러 번 긋기와 높이 고르기를 합쳤다.

- **row는 클라이언트가 칸 번호로 보낸다.** 칸 수는 `기둥 수 × 3`이고 PREPARE 메시지의 `rowCount`로 알린다. 모두가 한 구간에 3개씩 그어도 자리가 모자라지 않는 최솟값이다.
- **같은 높이 충돌은 거절한다.** 같은 높이에서 기둥을 공유하는 자리(같은 구간, 좌우 옆 구간)에 선이 있으면 무시한다. 결정 1이 "전역 삽입 순서라 충돌이 수학적으로 불가능하다"고 적었던 전제가 깨지므로 검사가 새로 필요하다.
- **검사와 추가는 `LadderLines.add` 한 임계 구역에서 한다.** `[laddergame]` 스트림은 공유 풀에서 처리되어 같은 자리를 두 요청이 동시에 노릴 수 있다. 늦은 요청은 `ROW_OCCUPIED`로 거절되고 Consumer가 무시한다.
- **경로 추적은 그대로 row 오름차순이다.** 같은 높이의 선은 기둥을 공유하지 않으므로 순서가 결과를 바꾸지 않는다.
- **결정 3의 ghost 보정이 사라진다.** row를 클라이언트가 정하므로 ghost가 처음부터 최종 위치에 뜬다. 선점당한 경우만 2초 타임아웃으로 지운다.

아래 대안 표의 "터치 위치로 row 결정"을 기각한 이유 두 가지는 이렇게 해소했다. 화면 좌표 대신 칸 번호를 보내서 서버가 좌표를 정규화할 필요가 없다. 동시 입력은 위 임계 구역으로 처리한다.
