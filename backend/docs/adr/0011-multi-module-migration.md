...
:room ──────────┘
  ↑
:game
:admin  → :global, :room, :user
:zzolbot→ :global, :room
:app    → all
```

### 3. `:global`을 분리하지 않는 이유
...
1. `settings.gradle.kts` 생성, 9개 모듈 등록
...
- `:websocket`이 `:room` 타입(PlayerKey 등)을 참조하는지 Phase 3에서 확인 필요. 참조한다면 인터페이스를 `:global`로 내리거나 이벤트 기반으로 역전
