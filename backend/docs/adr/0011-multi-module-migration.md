...
패키지 이름은 Phase 1 원칙에 따라 모두 변경하지 않았다.

### 계획 → 실제 이동 차이

**`:global` → `:websocket`** (websocket 타입 또는 SpringDoc 참조로 인한 이동)

| 원래 파일 경로                                             | 계획 모듈     | 실제 모듈        | 이유                                                                    |
|------------------------------------------------------|-----------|--------------|-----------------------------------------------------------------------|
| `global/config/SwaggerConfig.java`                   | `:global` | `:websocket` | `io.swagger.v3.oas.models.*` — SpringDoc이 `:websocket`에만 선언됨          |
| `global/exception/WebSocketExceptionHandler.java`    | `:global` | `:websocket` | `coffeeshout.websocket.LoggingSimpMessagingTemplate` 참조               |
| `global/health/GracefulShutdownHealthIndicator.java` | `:global` | `:websocket` | `coffeeshout.websocket.lifecycle.WebSocketGracefulShutdownHandler` 참조 |

**`:global` → `:user`** (user 타입 참조로 인한 이동)

| 원래 파일 경로                          | 계획 모듈     | 실제 모듈   | 이유                                                                  |
|-----------------------------------|-----------|---------|---------------------------------------------------------------------|
| `global/config/WebMvcConfig.java` | `:global` | `:user` | `coffeeshout.user.ui.resolver.AuthenticatedUserArgumentResolver` 참조 |
...
| 원래 파일 경로                                                   | 계획 모듈   | 실제 모듈   | 이유                                                                     |
|------------------------------------------------------------|---------|---------|------------------------------------------------------------------------|
| `user/application/service/UserNicknameCleanupService.java` | `:user` | `:room` | `coffeeshout.room.infra.event.ProfanityWordBlockedEvent` 참조            |
...
**`:room` → `:admin`** (dashboard 타입 참조로 인한 이동)

| 원래 파일 경로                                                                    | 계획 모듈   | 실제 모듈    | 이유                                                                  |
|-----------------------------------------------------------------------------|---------|----------|---------------------------------------------------------------------|
| `room/application/service/player/name/PlayerNameRankingCleanupService.java` | `:room` | `:admin` | `coffeeshout.dashboard.domain.*` 참조                                 |
| `room/infra/PlayerNameRankingCleanupScheduler.java`                         | `:room` | `:admin` | 위 서비스가 `:admin`으로 이동했으므로 해당 서비스를 주입하는 스케줄러도 함께 이동 |
...
**`:room` → `:zzolbot`** (Gemini AI 참조로 인한 이동)

| 원래 파일 경로                                  | 계획 모듈   | 실제 모듈      | 이유                                                |
|-------------------------------------------|---------|------------|---------------------------------------------------|
| `room/infra/GeminiPlayerNameAuditor.java` | `:room` | `:zzolbot` | `com.google.genai.*` — Gemini 의존성이 `:zzolbot`에 있음 |
...
- `zzolbot/src/main/java/coffeeshout/room/config/GeminiAuditConfig.java` — `RoomConfig`에서 분리한 Gemini `Client` bean (`@Profile("!local & !test")`)
- `websocket/src/main/java/coffeeshout/websocket/MessageRecoveryPort.java` — `GameRecoveryService`와 `LoggingSimpMessagingTemplate` 순환 의존 해소용 포트 인터페이스 (`save()` 메서드만 선언)
...
`GameRecoveryService.java` — `implements MessageRecoveryPort` 추가.

`LoggingSimpMessagingTemplate.java` — `GameRecoveryService` 필드 타입을 `MessageRecoveryPort`로 교체.

### build.gradle.kts 변경 (계획에 없던 의존성)

| 모듈           | 추가 의존성                                                                         | 이유                                                                              |
|--------------|--------------------------------------------------------------------------------|---------------------------------------------------------------------------------|
| `:global`    | `resilience4j-spring-boot3` (추가), `jspecify` (추가), ZXing·resilience4j `api` 승격 | `IpBlockStore`(resilience4j), `IpBlockFilter`(jspecify), 하위 모듈 컴파일 전파           |
| `:websocket` | `spring-boot-starter-security`, `jjwt-api/impl/jackson`                        | `WsCatalogSecurityConfig`, `websocket/auth/JjwtRoomSessionTokenIssuer`          |
...
| `room/infra/PlayerNameRankingCleanupScheduler.java` | `:admin`으로 이동한 `PlayerNameRankingCleanupService`를 찾지 못함 | 스케줄러도 `:admin` 모듈로 이동 |
