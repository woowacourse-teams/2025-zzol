...
### reuse 재활성화 — 모듈별 격리 복구 (2026-06-14)

위 영구 비활성화(#1402)는 철회한다. 게임 통합테스트 플레이키의 근본 원인을 "공유 컨테이너 자원
경합"으로 지목했으나, 그 판정은 잘못이었다. #1402 비활성화 시점(06-11)에 이미 트리에 있던
#1361/#1369 와 달리, **이후 머지된 #1411(게임 통합테스트 subscribe 등록 완료 대기, 타이밍 결함)**
이 실제 결함을 해소했다. 즉 플레이키의 원인은 reuse(공유 컨테이너)가 아니라 구독 등록과 메시지
발행 사이의 타이밍이었다.

따라서 `withReuse(true)`(JVM 간 단일 공유 컨테이너)와 모듈별 DB(`zzol_test_<module>`)·Redis
인덱스(0~8) 격리, `test.db.name` / `test.redis.db` 시스템 프로퍼티를 복구한다. 공유 컨테이너의
기동 시간 이점을 되찾으면서, 데이터 충돌은 모듈별 격리로 막는다.

- 이슈: #1417
- CI `backend-ci.yml` 의 "Enable Testcontainers reuse" 스텝(main·be/dev 양쪽에 잔존)이 이 복구로
  다시 유효해지므로 워크플로 변경은 불필요하다.
- 검증 한계: #1402 재현은 (a) Valkey stale 이벤트 오염과 (b) MySQL 자원 경합 두 모드를 보고했다.
  #1411 은 (a) 해소가 분명하나 (b)를 직접 다루지 않으므로, 이전 플레이키 게임 통합테스트가 CI 병렬
  실행에서 재발하지 않는지로 안정성을 확인한다(로컬 Docker 미가용 환경).
