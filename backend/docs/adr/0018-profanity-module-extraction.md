...
- AI 기반 감지(Perspective API)는 별도 모듈로 추후 도입한다
...
```text
domain/
  ProfanityWord        — JPA 엔티티 (word, language, source, is_active, match_count)
...
application/
  ...
  ProfanityWordSyncService    — ProfanityWordBlockedEvent 수신 → DB 저장 → trie 리빌드
...
| `AI_FLAGGED` | 한/영 | Perspective API 감지 후 승인된 단어 (Phase 2) |
...
- Perspective API 기반 AI 레이어(Phase 2)는 `:profanity-ai` 모듈로 분리해 독립 도입이 가능하다.
