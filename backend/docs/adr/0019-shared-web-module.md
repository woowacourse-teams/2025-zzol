...
1. **테스트 컨텍스트 불완전**: 도메인 모듈 단독 테스트 시 `RestExceptionHandler`가 로드되지 않아
   ...
   ADR-0018에서 `TestRestExceptionHandler`로 임시 대처했으나, 모듈이 늘어날수록 동일한
...
- `miniGameFactoryMap` mock → ADR-0018 대안 D(`:test-support` 집중 관리)로 별도 검토 예정
