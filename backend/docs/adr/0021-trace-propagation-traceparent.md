...
- **Outbox 경로**: `OutboxEventRecorder`가 **기록 시점에** `traceparent`를 outbox 레코드에 함께 저장한다. 재시도 릴레이(`OutboxRelayWorker`)는 스케줄러 스레드에서 실행되어 원본 트레이스 컨텍스트가 없으므로, 발행 시점 추출로는 trace가 유실된다

### 신구 메시지 호환

- 구형 메시지(JSON에 `traceInfo` 필드 잔존)를 신형 코드가 읽는 경우: `redisObjectMapper`가 `FAIL_ON_UNKNOWN_PROPERTIES=false`라 무해하다
- 구형 레코드 구조(`ObjectRecord` 단일 필드)를 신형 리스너가 읽는 경우: 레거시 필드 폴백을 1릴리스 동안 유지한 뒤 제거한다
...
- `MiniGameBaseEvent`의 중복 계약(`eventId()`/`timestamp()`/`traceInfo()`)을 `BaseEvent`로 통합
...
- 샘플링 결정이 producer→consumer로 표준 전파된다 (`sampled(true)` 강제 제거)
- 메시지 처리 흐름(Handler → Stream → Consumer)은 변경 없음 — 전파 메커니즘만 교체
- `docs/architecture.md`, `docs/tech-stack.md`의 TraceInfo 언급을 구현 시점에 갱신한다
