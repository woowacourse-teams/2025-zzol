...
## 개정 (2026-06-16) — Alloy state 볼륨 영속화

결정 3은 compose 볼륨을 `app-logs`(`:ro`)·`localtime` 2개로 한정 열거하고 "기존 값을 그대로 유지한다"고 기술했다. 그러나 command의 `--storage.path=/var/lib/alloy/data`(positions·WAL)를 받칠 볼륨이 없어, **컨테이너 재생성 시 수집 state가 유실되고 Loki에 로그가 중복 적재될 수 있었다**(코드 리뷰 지적). 이를 다음과 같이 교정한다.

- dev/prod 각각 named volume `dev-alloy-data`/`prod-alloy-data`를 `/var/lib/alloy/data`에 마운트하고, 전역 `volumes:` 섹션에 `driver: local`로 선언한다. 결정 3의 볼륨 목록은 이제 3개다(`app-logs`·`localtime`·`*-alloy-data`).
- 이에 따라 결과·체크리스트의 **"promtail은 별도 볼륨 없이 positions를 `/tmp`에 두므로 상태 마이그레이션 불필요"** 전제는 Alloy 전환 후 무효화된다. Alloy는 positions·WAL을 영속 볼륨에 보관하므로, 향후 정리 시 `*-alloy-data` 볼륨을 함께 고려한다.
- 이미지 핀(`v1.16.3`)·`--storage.path`·dev/prod 타임스탬프 처리·healthcheck 미추가 등 나머지 결정은 불변이다.
