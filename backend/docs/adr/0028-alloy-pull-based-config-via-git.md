...
- 날짜: 2026-06-12
- 상태: 승인 (#1416 머지, prod 적용은 조건부)
- 2026-07-17 갱신: 추종 브랜치를 통합 브랜치로 이관(#1574) — dev는 `be/dev`→`dev`, prod는 `be/prod`→`prod`. 본문의 `be/dev`·`be/prod`는 각각 현재 `dev`·`prod`에 해당한다 — PR 게이트 구조는 동일.
...
- **배포 후 확인**:
  - **① Loki 로그 인입.** Grafana Explore(Loki)에서 `{job="dev-app"}`(prod은 `{job="prod-app"}`)을 조회해 최근 로그가 보이는지, prod은 타임스탬프가 KST→UTC로 변환됐는지 확인한다.
  - **② Prometheus 스크레이프.** `up{job="alloy"}==1`. 이는 컨테이너 기동(`docker ps`)이 아니라 **Prometheus→`alloy:12345` 스크레이프 성공**을 검증한다 — `AlloyDown` 알림이 성립하는 전제이며 `deploy-infrastructure.sh`의 `check_container_running`(프로세스 존재만 확인)으로는 대체되지 않는다.
  - **③ 핫리로드.** `backend/docker/alloy/modules/app-logs.alloy`의 `app_logs_ingest_time` → `loki.write "default"`의 `external_labels`를 `{ reload_test = "1" }`로 임시 변경해 be/dev에 푸시 → 60초 내 Grafana(Loki)에서 `{reload_test="1"}`(dev-app 스트림)이 잡히면 핫리로드 동작. 확인 후 `external_labels = {}`로 되돌려 푸시.
  - **④ github egress.** ①③이 통과하면 배포 시점 egress는 이미 검증된 것이다(별도 확인 불필요). 이후 지속 의존(GitHub 장애·방화벽 변경으로 pull 중단)은 `AlloyComponentUnhealthy`가 사후 감지한다 — 이때 Alloy는 last-good config로 계속 동작한다.
...
### 5) prod 승격·배포 전 체크리스트 + 복구

prod Alloy bootstrap은 `revision = "be/prod"`로 모듈을 pull하므로(dev는 `be/dev`), prod 배포 **전**에 다음을 확인한다:

- [ ] `backend/docker/alloy/modules/app-logs.alloy`가 **be/prod에 존재**한다: `git cat-file -e origin/be/prod:backend/docker/alloy/modules/app-logs.alloy`.
- [ ] `backend/docker/prod/conf/config.alloy`의 `revision`이 **`be/prod`**를 가리킨다.
- [ ] 배포 순서(be/dev 머지 → be/prod 승격 → prod 배포)를 지켰다.

**복구 — prod 배포 시점에 be/prod에 모듈이 없을 때**: prod-alloy의 import.git이 실패해 prod 로그가 끊긴다(컨테이너는 살아 있으나 `AlloyComponentUnhealthy`). 해소는 **모듈 파일을 포함한 PR로 be/prod를 승격**한다 — be/prod는 보호 브랜치라 직접 커밋·cherry-pick은 금지, 반드시 PR을 경유한다. be/prod에 모듈이 들어오면 import.git이 다음 `pull_frequency`(60s)에 자동 재시도하므로 컨테이너 재시작 없이 복구될 수 있다(즉시 반영이 필요하면 prod 재배포).
