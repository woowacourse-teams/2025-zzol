...
1. **전원 입력 완료** — 모든 플레이어가 (정상/충돌로) 입력을 마치면 종료한다. 단 마지막 입력
   ...
   마지막 사람이 서는 순간 화면이 즉시 다음 단계로 튀는 것을 막기 위함이다(결정 9의 "결과를 잠깐
   보여주고 전이" 패턴을 종료 직전에도 적용). 지연 동안에는 입력·윈도우·idle·쿨다운 타이머를 멈추고
...
- 타이밍 설정(외부화): `nunchi.timing.*` — 동시 윈도우(300ms)·충돌 쿨다운·무입력 idle 타임아웃
  (예 10초)·하드 안전 캡(예 30초)·전원 입력 후 종료 대기(`allPressedDelay`)·종료 후 결과 대기
  (`resultDelay`, 결정 9)
