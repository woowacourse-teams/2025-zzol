...
1. **전원 입력 완료** — 모든 플레이어가 (정상/충돌로) 입력을 마치면 종료한다. 단 마지막 입력
   직후 곧장 `DONE`으로 넘기지 않고, 마지막 입력을 잠깐 보여준 뒤(`allPressedDelay`) 종료한다 —
   마지막 사람이 서는 순간 화면이 즉시 `DONE`으로 튀는 것을 막기 위함이다. 지연 동안에는 입력·윈도우·idle·쿨다운 타이머를 멈추고
   단발 종료 타이머만 남긴다. idle·하드 캡 타임아웃(2·3)은 이미 "시간이 다 된" 종료라 이 지연 없이
   곧바로 종료한다.
...
### 9. 시작 규칙 설명(DESCRIPTION) + 곧 시작 카운트다운(READY) 단계 — 2026-06-24 개정
...
**종료 후 전이 — `result-delay` 버퍼 제거(2026-06-24 개정)**: 게임이 끝나면 `finish()`가 `DONE`을 즉시
브로드캐스트하고 곧바로 `finalizeGame`(`finishGame()`으로 roundCount 확정 → `MiniGameFinishedEvent`
발행 → 확률 조정·SCORE_BOARD 전이)을 **같은 락 아래 동기 수행**한다. 이전 개정에서 두었던
`result-delay`(`nunchi.timing.result-delay`) 결과 대기 버퍼는 제거했다 — 종료 결과 표시는 다음
단계(SCORE_BOARD)가 담당하므로 서버측 추가 지연을 두지 않는다. 순서 불변식(ADR-0025 결정 5)은
그대로다: `finishGame()`으로 roundCount를 확정한 뒤 이벤트를 발행한다.
...
- 타이밍 설정(외부화): `nunchi.timing.*` — 규칙 설명(`description`)·곧 시작 카운트다운(`ready`)·동시
  윈도우(300ms)·충돌 쿨다운·무입력 idle 타임아웃(예 10초)·하드 안전 캡(예 30초)·전원 입력 후 종료
  대기(`allPressedDelay`) (결정 9 — `result-delay`는 제거)
