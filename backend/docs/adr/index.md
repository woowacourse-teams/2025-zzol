...
| [0011](0011-websocket-catalog-discovery.md) | WebSocket 컨트랙트 디스커버리 — `@WsTopic`/`@WsQueue` + `/dev/ws-catalog` | 보류 | `global/websocket/docs/`, 9개 도메인 컨트롤러/Publisher, `friend/application/FriendNotifier`, `friend/application/PresenceNotifier` | 토픽 발행 메서드는 `@WsTopic`, 비동기 핸들러는 `@WsReceive`, 개인 큐 발행 메서드는 `@WsQueue` 사용 |
