...
| [0011](0011-websocket-catalog-discovery.md) | WebSocket 컨트랙트 디스커버리 — `@WsTopic`/`@WsQueue`/`@WsReceive` + `/dev/ws-catalog` | 보류 | `global/websocket/docs/`, 도메인별 컨트롤러/Publisher/Notifier (총 15개), `friend/application/FriendNotifier`, `friend/application/PresenceNotifier` | 토픽 발행은 `@WsTopic`, 개인 큐 발행은 `@WsQueue`, 비동기 핸들러는 `@WsReceive` 사용. `Object.class` payload 금지. 동일 path 다중 발행자는 `publishers` 배열로 묶임 |
