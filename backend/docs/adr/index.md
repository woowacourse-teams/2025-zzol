...
| [0003](adr/0003-oauth2-login-and-user-domain.md) | OAuth2 로그인 도입과 User 도메인 분리 | 승인 | `user/`, `room/infra/persistence/`, `auth/` | Player-User는 nullable FK로 분리, 회원 입장 시 프로필 닉네임 강제, 익명 흐름 그대로 유지 |
| [0004](adr/0004-jwt-refresh-token-policy.md) | Access(JWT) + Refresh(Redis) 인증 토큰 정책 | 승인 | `user/config/`, `user/application/service/`, `user/infra/redis/` | Refresh token은 `{userId}:{tokenId}` 형식, 재사용 감지 시 family 전체 폐기 |
| [0005](adr/0005-user-code-policy.md) | 5자리 사용자 식별 코드(UserCode) 정책 | 승인 | `user/domain/`, `user/domain/service/` | UserCode는 가입 시 1회 발급 후 불변, INSERT-then-catch + 최대 100회 재시도 |
