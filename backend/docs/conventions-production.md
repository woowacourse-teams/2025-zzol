...
각 항목의 **강제 주체**를 괄호로 적는다. `PMD`는 CI에서 자동 판정하고(룰셋: `config/pmd/ruleset-main.xml`,
근거: [ADR-0036](adr/0036-static-analysis-lint-stack.md)), 나머지는 리뷰가 본다.

- 클래스는 단일 책임을 갖는다. 변경 이유가 두 가지 이상이라면 클래스를 분리한다 (리뷰)
- 인스턴스 변수는 가능한 한 `final`로 선언한다 (리뷰 — JPA 엔티티는 non-final이 필수라 기계 판정 불가)
- 메서드 블록 내 지역 변수에는 `final`을 붙인다. 매개변수에는 붙이지 않는다 (리뷰 — 위반 1,346곳이라 규칙화는 별도 이슈)
- `if-else` 사용을 지양하고 early return으로 중첩을 줄인다 (**PMD** `NoElseAfterReturn` — then이 `return`/`throw`로 끝나는 경우)
- 들여쓰기 깊이가 깊어지면 메서드로 분리한다. 한 메서드는 한 가지 추상화 수준의 일만 한다
  (**PMD** `MaxNestingDepth`(현재 3, 목표 2)·`NcssCount`(현재 문장 25, 목표 20))
- `@Autowired` 필드 주입 대신 생성자 주입을 쓴다 (**PMD** `NoFieldInjection` — 테스트 소스는 제외)
- 표준 출력(`System.out`)·`printStackTrace` 대신 로거를 쓴다 (**PMD**)
- 도메인 계층에서 `catch (Exception)`으로 뭉뚱그리지 않는다 (**PMD** `NoGenericCatchInDomain` — application 경계는 제외)
