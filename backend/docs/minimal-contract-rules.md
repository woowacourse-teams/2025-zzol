...
**충족 예** — 전이마다 `version` 증가, `WATCH state:{joinCode}` 후 MULTI, 실패 시 재조회·재시도. / **위반 예** — `redis.set(key, serialize(game))`로 무조건 덮어쓰기.
...
**계약** — 게임 진행 중 입력·타이머 처리 경로의 동기 왕복은 Redis까지만 허용한다. RDB(MySQL) 동기 접근은 정산·결과 영속 등 게임 루프 밖 경로에서만 한다.

**근거** — 신설. 카드 선택은 초 단위 응답성이 필요하고, 결과 영속은 현행도 게임 종료 후 이벤트 경로에서 수행한다(`MiniGameResultSaveEventListener`).

**판정 기준**

- 카드 선택·페이즈 전이 처리의 동기 경로에 JPA/JDBC 호출이 있으면 위반.
- 게임 종료·정산 시점의 DB 영속은 충족(위반 아님).

**검증** — `llm` (`script` 보조 가능: 핫패스 패키지에서 repository 의존 grep).
