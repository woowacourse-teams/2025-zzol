...
- `calculateProbabilityChange(int, int)`: 등수와 동점자 수로 조정할 확률을 계산한다.

## 룰렛 범위들 (RouletteRanges)

### 상태

- `ranges`: `List<RouletteRange>` 플레이어별 확률로 계산한 범위 정보
...
- `of(int, int)`: 전체 플레이어 수와 순위로 결과 타입을 반환한다.
