...
- `applyMiniGameResult(Playable)`: 미니게임 결과를 룰렛에 적용한다. `shouldAdjustProbabilities()`가 false이면 확률 조정을 건너뛴다.
...
- `setUp(List<Player>)`: 게임에 플레이어를 설정한다. (`Room.startNextGame()` 에서 호출)
- `shouldAdjustProbabilities()`: 게임 종료 후 룰렛 확률을 조정할지 여부를 반환한다. (기본값 `true`)
