...
public class BlockStackingNotifier {
    ...
    @WsTopic(
            path = "/room/{joinCode}/block-stacking/state",
            payload = BlockStackingStateResponse.class,
    ...
    public void notifyPlayingStarted(JoinCode joinCode, Instant playingEndTime) {
        messagingTemplate.convertAndSend(
                String.format(STATE_DESTINATION_FORMAT, joinCode.getValue()),
                WebSocketResponse.success(BlockStackingStateResponse.ofPlaying(playingEndTime.toEpochMilli())));
    }

    @WsTopic(
            path = "/room/{joinCode}/block-stacking/progress",
            payload = BlockStackingProgressResponse.class,
