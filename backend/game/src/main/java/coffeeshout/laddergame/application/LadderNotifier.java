...
public class LadderNotifier {
    ...
    @WsTopic(
            path = "/room/{joinCode}/ladder/state",
    ...
    @WsTopic(
            path = "/room/{joinCode}/ladder/line",
            payload = LadderLineResponse.class,
    ...
    private void sendState(JoinCode joinCode, LadderStateResponse response) {
        messagingTemplate.convertAndSend(
                String.format(STATE_DESTINATION_FORMAT, joinCode.getValue()), WebSocketResponse.success(response));
    }
}
