...
public class MiniGameRestController implements MiniGameApi {
    ...
    /**
     * 존재하지 않는 방은 404로 거부한다(이전 {@code :room} 엔드포인트의 응답 계약 유지). 방 생성 시
     * {@code GameSessionInitConsumer}가 세션을 사전 생성하므로(ADR-0025 결정 6) 세션 존재로 방 존재를
     * 판별한다 — {@code :room} 조회 의존 없이 동일한 404 계약을 유지한다(ADR-0034).
     */
    private JoinCode validateRoomExists(String joinCode) {
        final JoinCode code = new JoinCode(joinCode);
        if (!gameSessionService.existsByJoinCode(code)) {
