...
/**
 * 눈치게임 상태 브로드캐스트(ADR-0031 결정 8). 상태 머신 {@code PLAYING ↔ COLLISION_COOLDOWN → DONE}을
 * 한 DTO로 표현하고, {@code @JsonInclude(NON_NULL)}로 상태별 무관 필드를 JSON에서 제외해 컨트랙트에
 * 정확히 맞춘다. 모든 타이밍은 서버 epoch ms이며 모든 메시지에 {@code serverNowEpochMs}(스큐 보정)를 싣는다.
 *
 * <ul>
...
public record NunchiStateResponse(
    ...
    public static NunchiStateResponse playing(
        ...
        return new NunchiStateResponse(
                ...
                null, null,
                serverNowEpochMs, idleDeadlineEpochMs, hardCapEpochMs, null);
    }
    ...
    public static NunchiStateResponse collisionCooldown(
        ...
        return new NunchiStateResponse(
                ...
                number, collided,
                serverNowEpochMs, null, null, resumeAtEpochMs);
    }

    public static NunchiStateResponse done() {
        return new NunchiStateResponse(
                NunchiState.DONE, null, null, null, null, null, null, null, null);
    }
}
