...
public class NunchiWebSocketController {
    ...
    @WsReceive(
            respondsOnTopics = {"/room/{joinCode}/nunchi/stand", "/room/{joinCode}/nunchi/state"},
            description = "눈치게임 press — 서버 수신 권위 시각(Q1)을 찍어 스트림 발행, 컨슈머가 충돌·순위 판정")
    public void press(@DestinationVariable String joinCode, Principal principal) {
        ...
        log.debug(
                "눈치게임 press 발행: joinCode={}, playerName={}, eventId={}",
                joinCode,
