...
public class SpeedTouchPlayers {
    ...
    public SpeedTouchPlayers(List<Gamer> gamers) {
        this.players = List.copyOf(gamers.stream().map(SpeedTouchPlayer::new).toList());
    }
    ...
    public SpeedTouchPlayer findByName(String name) {
        return players.stream()
                ...
                .findFirst()
                .orElseThrow(() ->
                        new SystemException(GlobalErrorCode.INTERNAL_SERVER_ERROR, "해당 플레이어를 찾을 수 없습니다: " + name));
    }
    ...
    public Map<Gamer, SpeedTouchPlayer> toGamerMap() {
        return players.stream().collect(Collectors.toMap(SpeedTouchPlayer::getGamer, p -> p));
    }
