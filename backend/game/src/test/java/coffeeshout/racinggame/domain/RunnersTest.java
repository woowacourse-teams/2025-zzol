...
class RunnersTest {
    ...
    void 플레이어의_속도를_업데이트할_수_있다() {
        // when
        runners.updateSpeed(players.getFirst().name(), 5, speedCalculator, Instant.now());
    ...
    void 우승자를_찾을_수_있다() {
        ...
        runners.updateSpeed(players.getFirst().name(), 10, speedCalculator, now);
        for (int i = 0; i < 100; i++) {
            runners.moveAll(now);
        }
    ...
    void 모든_러너의_속도를_조회할_수_있다() {
        // given
        runners.updateSpeed(players.getFirst().name(), 8, speedCalculator, Instant.now());
        runners.updateSpeed(players.get(1).name(), 8, speedCalculator, Instant.now());
    ...
    void 모든_러너가_완주했는지_확인할_수_있다() {
        ...
        final Instant now = Instant.now();
        runners.updateSpeed(players.getFirst().name(), 10, speedCalculator, now);
        runners.updateSpeed(players.get(1).name(), 10, speedCalculator, now);
        for (int i = 0; i < 100; i++) {
            runners.moveAll(now);
        }
