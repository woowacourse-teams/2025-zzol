...
class MaliciousPathMatcherTest {
    ...
    class 악성_경로_감지 {
        ...
        @ParameterizedTest(name = "{0}")
        @ValueSource(strings = {"/.ENV", "/.Git", "/WP-ADMIN", "/XMLRPC.PHP"})
    ...
    class 정상_경로_허용 {
        ...
        @ParameterizedTest(name = "{0}")
        @ValueSource(
                strings = {"/", "/reports", "/admin", "/admin/login", "/admin/reports", "/ws", "/api/rooms", "/health"})
    ...
    class 엣지_케이스 {
        ...
        @ParameterizedTest(name = "{0}")
        @ValueSource(
                strings = {"/%2eenv", "/.git%2fconfig", "/%77p-admin" // 'w'의 URL 인코딩
                })
