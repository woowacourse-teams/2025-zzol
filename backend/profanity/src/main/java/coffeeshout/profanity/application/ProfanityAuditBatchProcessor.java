...
public class ProfanityAuditBatchProcessor {
    ...
    /**
     * @param sampler 이번 회차의 CLEAN 표본 추출기. 회차가 하나 만들어 모든 배치에 같은 것을 넘긴다.
     */
    public int process(List<NicknameAudit> batch, CleanSampler sampler) {
        ...
        try {
            final Integer settled = settlePhaseTimer.record(
                    () -> transactionTemplate.execute(status -> settle(batch, nicknames, resultMap, sampler)));
            ...
            return settleIndividually(batch, resultMap, sampler);
        }
    }
    ...
    private int settleIndividually(
            List<NicknameAudit> batch, Map<String, NicknameAuditResult> resultMap, CleanSampler sampler) {
        int settled = 0;
        for (final NicknameAudit entity : batch) {
            ...
            try {
                transactionTemplate.executeWithoutResult(status -> {
                    applyResult(entity, result, sampler);
    ...
    private void applyResult(NicknameAudit entity, NicknameAuditResult result, CleanSampler sampler) {
        entity.complete(result.status(), result.confidence(), result.reason());
        if (result.status() == NicknameAuditStatus.FLAGGED) {
            autoBlock(result);
        }
        // 벌크 저장이 실패하면 건별 폴백이 같은 엔티티에 다시 반영하지만, 이미 표본인 행은 예산을 쓰지 않는다.
        // 표시는 엔티티에 남아 있으므로 폴백의 save가 그대로 저장한다.
        sampler.trySample(entity);
    }
