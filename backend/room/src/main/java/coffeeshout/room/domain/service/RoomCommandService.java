...
public class RoomCommandService {
    ...
    public void markPlaying(JoinCode joinCode, PlayerName hostName) {
        log.info("JoinCode[{}] 게임 시작 상태로 전환 - 호스트: {}", joinCode, hostName);
        final Room room = roomQueryService.getByJoinCode(joinCode);
        room.markPlaying(hostName);
        save(room);
    }

    public void applyGameResult(JoinCode joinCode, Map<PlayerName, Integer> rankByPlayer, int totalGameCount) {
        log.info("JoinCode[{}] 게임 결과 반영 - totalGameCount: {}", joinCode, totalGameCount);
        final Room room = roomQueryService.getByJoinCode(joinCode);
        room.applyGameResult(rankByPlayer, totalGameCount);
        save(room);
    }
}
