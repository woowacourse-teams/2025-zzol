 ...
 *
 * <p>:game이 미니게임 결과를 {@code Long} FK로 저장하려면 room_session id와 이름별 player id가 필요하다.
...
class RoomSnapshotQueryAdapterIntegrationTest extends RoomModuleIntegrationTest {
    ...
    @Test
    @DisplayName("resolveRoomSessionId는 joinCode의 room_session id를, resolvePlayers는 이름별 playerId를 반환한다")
    void 방과_플레이어_식별자를_조회한다() {
        // given — RoomEntity + PlayerEntity를 실제 DB에 커밋한다
