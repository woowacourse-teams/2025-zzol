...
public class BlockStackingCommandEventConsumer implements Consumer<BlockStackingCommandEvent> {
    ...
    public void accept(BlockStackingCommandEvent event) {
        try {
            blockStackingService.recordProgress(event.joinCode(), event.playerName(), event.floor(), event.tapX(),
                    event.movingBlockX(), event.stackTopX(), event.stackTopWidth());
        } catch (BusinessException e) {
            log.warn(
                    "블록 쌓기 명령 이벤트 처리 중 비즈니스 예외 발생: joinCode={}, playerName={}, floor={}, tapX={}, movingBlockX={}, stackTopX={}, stackTopWidth={},",
                    event.joinCode(), event.playerName(), event.floor(), event.tapX(), event.movingBlockX(),
                    event.stackTopX(), event.stackTopWidth(), e);
        } catch (Exception e) {
            log.error(
                    "블록 쌓기 명령 이벤트 처리 중 오류 발생: joinCode={}, playerName={}, floor={}, tapX={}, movingBlockX={}, stackTopX={}, stackTopWidth={}, ",
                    event.joinCode(), event.playerName(), event.floor(), event.tapX(), event.movingBlockX(),
                    event.stackTopX(), event.stackTopWidth(), e);
        }
    }
