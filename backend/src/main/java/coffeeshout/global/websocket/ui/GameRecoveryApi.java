...
public interface GameRecoveryApi {
    ...
    ResponseEntity<RecoveryResponse> requestRecovery(
            ...
            @Parameter(description = "플레이어 이름", required = true) @RequestParam String playerName,
            @Parameter(description = "마지막으로 수신한 메시지 ID (Redis Stream ID)", required = true) @RequestParam String lastId
    );
}
