...
public class NumberPokerFlowOrchestrator {
    ...
    private final NumberPokerProbabilityProperties probabilityProps;
    ...
    public void startFlow(NumberPokerGame game, Room room) {
        ...
        for (int i = 0; i < totalRounds; i++) {
            ...
            flow = chainRound(flow, game, room, allFoldedTrigger, readyTrigger, isLast);
        }

        flow.andThen(finishGame(game, room), Duration.ZERO)
                .onError(ex -> {
    ...
    private FlowHandle chainRound(FlowHandle flow, NumberPokerGame game, Room room,
                                   ...
                                   boolean isLast) {
        // STAGE_1
        flow = flow.andThen(step(game, room, NumberPokerGame::beginStage1), Duration.ZERO);
    ...
    private Map<Player, Integer> applyRoundProbabilities(NumberPokerGame game, Room room) {
        ...
        final Map<Player, Integer> deltas = probabilityAdjuster.calculate(
                results,
                probabilityProps.adjustmentStep(),
                game.getTotalRounds()
        );
