...
class BlockStackingGameTest {
    ...
    class 진행_기록_테스트 {
        ...
        void 유효한_탭_이벤트_기록_시_true를_반환하고_floor가_증가한다() {
            final boolean recorded = game.recordProgress(꾹이, 1, MOVING_BLOCK_X, MOVING_BLOCK_X, STACK_TOP_X,
                    STACK_TOP_WIDTH);
        ...
        void 여러_층을_순서대로_쌓을_수_있다() {
            game.recordProgress(꾹이, 1, MOVING_BLOCK_X, MOVING_BLOCK_X, STACK_TOP_X, STACK_TOP_WIDTH);
            game.recordProgress(꾹이, 2, MOVING_BLOCK_X, MOVING_BLOCK_X, STACK_TOP_X, STACK_TOP_WIDTH);
            game.recordProgress(꾹이, 3, MOVING_BLOCK_X, MOVING_BLOCK_X, STACK_TOP_X, STACK_TOP_WIDTH);
        ...
        void 비연속적인_floor_수신_시_false를_반환하고_floor가_갱신되지_않는다() {
            ...
            final boolean recorded = game.recordProgress(꾹이, 2, MOVING_BLOCK_X, MOVING_BLOCK_X, STACK_TOP_X,
                    STACK_TOP_WIDTH);
        ...
        void overlap이_0인_경우_false를_반환하고_floor가_갱신되지_않는다() {
            ...
            final boolean recorded = game.recordProgress(꾹이, 1, 300.0, 300.0, STACK_TOP_X, STACK_TOP_WIDTH);
        ...
        void overlap이_음수인_경우_false를_반환한다() {
            ...
            final boolean recorded = game.recordProgress(꾹이, 1, -100.0, -100.0, STACK_TOP_X, STACK_TOP_WIDTH);
        ...
        void 각_플레이어는_독립적으로_floor를_쌓는다() {
            game.recordProgress(꾹이, 1, MOVING_BLOCK_X, MOVING_BLOCK_X, STACK_TOP_X, STACK_TOP_WIDTH);
            game.recordProgress(꾹이, 2, MOVING_BLOCK_X, MOVING_BLOCK_X, STACK_TOP_X, STACK_TOP_WIDTH);
            game.recordProgress(루키, 1, MOVING_BLOCK_X, MOVING_BLOCK_X, STACK_TOP_X, STACK_TOP_WIDTH);
    ...
    class 점수_결과_테스트 {
        ...
        void 층수가_같은_플레이어는_동일_순위를_받는다() {
            game.recordProgress(엠제이, 1, MOVING_BLOCK_X, MOVING_BLOCK_X, STACK_TOP_X, STACK_TOP_WIDTH);
            // 루키=1층, 엠제이=1층 → 공동 2위
