...
public class PresenceNotifier {
    ...
    @WsQueue(path = PRESENCE_QUEUE, payload = PresencePayload.class,
            description = "친구 접속·방 참여 상태 변경 알림")
    ...
    private void broadcastToFriends(PresencePayload payload) {
        friendshipService.findAcceptedFriendIds(payload.userId())
                .forEach(friendId -> sendTo(friendId, payload, "Presence 알림"));
    }

    private RoomMembership findMembership(Long userId) {
        return roomMembershipQuery.findByUserIds(List.of(userId))
                .getOrDefault(userId, RoomMembership.NONE);
    }
    ...
    private void sendTo(Long addresseeId, PresencePayload payload, String logLabel) {
        try {
            ...
            log.debug("{}: userId={}, online={}, joinCode={}, 수신자={}",
                    logLabel, payload.userId(), payload.online(), payload.joinCode(), addresseeId);
        } catch (Exception e) {
            log.warn("{} 전송 실패: userId={}, 수신자={}, 원인={}",
                    logLabel, payload.userId(), addresseeId, e.getMessage());
        }
    }
}
