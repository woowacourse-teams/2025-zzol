...
public class WebSocketExceptionHandler {
    ...
    public void handleBusinessException(
            CoffeeShoutException e, Principal user, @Header("simpDestination") String destination) {
        log.warn(
                "WebSocket BusinessException: destination={}, errorCode={}, message={}",
                destination,
                e.getErrorCode().getCode(),
                e.getMessage());

        messagingTemplate.convertAndSendError(user.getName(), e.getErrorCode().getMessage());
    }

    @MessageExceptionHandler(MethodArgumentNotValidException.class)
    public void handleValidationException(
            MethodArgumentNotValidException e, Principal user, @Header("simpDestination") String destination) {
        final String errors = e.getBindingResult().getAllErrors().stream()
                .map(error -> error instanceof FieldError fe
                        ? fe.getField() + ": " + fe.getDefaultMessage()
                        : (error.getCode() != null ? error.getCode() : error.getObjectName()) + ": "
    ...
    public void handleException(Exception e, Principal user, @Header("simpDestination") String destination) {
        log.error("WebSocket Exception: destination={}, message={}", destination, e.getMessage(), e);

        messagingTemplate.convertAndSendError(user.getName(), "처리 중 오류가 발생했습니다.");
    }
}
