...
**장기 목표를 `dev`(통합 개발) → `prod`(통합 배포) 2-브랜치로 단일화**하고, `be/dev`·`fe/dev`·`be/prod`·`fe/prod` 레거시 4브랜치를 폐기한다. 단, 위와 같은 분기(양방향 차이) 때문에 **일괄 병합이 아니라 단계적 전환**으로 진행한다.

전환 순서(초안):

1. **차이 맞추기 선행** — `dev`와 `be/prod`·`fe/prod` 사이 양방향 차이를 해소한다. prod에만 있는 커밋(be +24 / fe +440)을 `dev`로 흡수하고, 검증 안 된 대형 변경이 prod로 새어나가지 않도록 범위를 확인한다. FE(2110파일)는 별도 차이 맞추기 작업이 필요하다.
2. **통합 `prod` 생성** — 차이 맞추기가 끝난 `dev`에서 `prod` 브랜치를 만든다.
...
| 대안 | 장점 | 단점 |
| --- | --- | --- |
| 일괄 `dev → be/prod` 병합 | 즉시 단일화 | dev의 미검증 19+커밋·838파일을 prod에 통째 배포 → 파급 범위 과다, 장애 위험(#1537 논의에서 기각) |
| 전환기 유지(현행) | 추가 작업 없음 | 풀스택/공통 수정마다 이원 배포·핫픽스, dev-prod 표류 지속 |
| **단계적 전환(채택)** | 차이 맞추기로 파급 범위 통제, 각 단계 검증 가능 | 여러 후속 이슈로 나뉘어 시간 소요, FE 차이 맞추기 부담 |

## 트레이드오프

- 단계적 전환은 즉시 끝나지 않는다. 특히 FE의 2110파일 분기를 맞추는 비용이 크다. 그러나 프로덕션에 미검증 릴리스를 흘리지 않는 안전성을 우선한다.
