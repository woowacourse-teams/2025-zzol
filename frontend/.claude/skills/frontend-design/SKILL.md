...
- 로그인·대기실·게임·결과처럼 에너지가 높은 상태 전환이 많은 서비스
...
2. **디자인 방향 결정**
   - 아래 방향 중 하나를 선택하고 이유를 한 줄로 명시
     - **임팩트형**: 포인트 컬러 강조, 굵은 타이포, 큰 여백. CTA·결과 화면에 쓴다
     - **긴장감형**: 어두운 배경, 레드와 화이트 대비, 미묘한 애니메이션. 게임 진행 중에 쓴다
     - **클린형**: 회색 계열, 작은 타이포, 최소 장식 — 설정·목록·폼
     - **축제형**: 포인트 컬러 + yellow 액센트, 스태거 애니메이션. 당첨·완료 상태에 쓴다
