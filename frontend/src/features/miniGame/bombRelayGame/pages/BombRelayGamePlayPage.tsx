...
const BombRelayGamePlayPage = () => {
  ...
  const myPlayerData = useMemo(
    () => progressData.players.find((p) => p.playerName === myName),
    [progressData.players, myName]
  );
  const amIEliminated = myPlayerData?.eliminated ?? false;
  const myEliminatedRound = myPlayerData?.eliminatedRound ?? 0;

  const isMyTurn = !amIEliminated && currentTurnPlayerName === myName;
  ...
  return (
    <Layout padding="0">
      ...
      <Layout.Content>
        <S.Container $isMyTurn={isMyTurn && isPlaying} $isEliminated={amIEliminated}>
          <S.RoundSection>
            <RoundInfo currentRound={currentRound} maxRounds={maxRounds} />
          </S.RoundSection>
          {isPlaying && amIEliminated && (
            <S.EliminatedBanner>
              💥 <S.EliminatedLabel>라운드 {myEliminatedRound}에서 탈락!</S.EliminatedLabel> 관전
              중...
            </S.EliminatedBanner>
          )}
          {isPlaying && !amIEliminated && (
          ...
          <S.InputSection>
            {amIEliminated ? (
              <S.SpectatingText>💀 탈락하여 관전 중입니다</S.SpectatingText>
            ) : (
